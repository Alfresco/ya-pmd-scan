...
with open(pmd_summary_path, 'w') as pmd_summary_file:
    ...
    for rule, delta in (new_violation_count - old_violation_count).most_common():
        log(f'\n+++ {delta} new {rule} violation(s) +++\n', pmd_summary_file)
        # Shortcut for the case that all the violations of this type are new.
        if len(new_details[rule]) == delta:
    ...
    for rule, delta in (old_violation_count - new_violation_count).most_common():
        log(f'\n--- {delta} resolved {rule} violation(s) ---\n', pmd_summary_file)
